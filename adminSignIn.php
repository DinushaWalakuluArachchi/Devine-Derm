<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="bootstrap.css" />
    <link rel="stylesheet" href="style.css" />
    <title>Divine Derm</title>
</head>


<body>
  
    <div class="adminsignInBody">

        <div class="adminSignIn_Box">

            <h2 class="text-center text-white">Admin Login</h2>

            <div class="mt-3 text-white">
                <label for="form-label">Username :</label>
                <input type="text" class="form-control border-black " placeholder=" Ex: Amal" id="un" />
            </div>

            <div class="mt-3 mb-3 text-white">
                <label for="form-label">Password :</label>
                <input type="password" class="form-control border-black " placeholder=" Ex: ********" id="pw" />
            </div>

            <div class="d-none" id="msgDiv">
                <div class="alert alert-danger" id="msg"></div>
            </div>

            <div class="mt-4">
                <button class="btn btn-secondary col-12 col-md-12 col-lg-12" onclick="adminSignIn();">Sign In</button>
            </div>

        </div>
    </div>


    <script src="script.js"></script>
</body>

</html>