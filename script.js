    ...
    if ((request.readyState == 4) & (request.status == 200)) {
      ...
      if (response == "Success") {
        document.getElementById("msg3").innerHTML =
          "Color Registration Successfully";
...
function updateStock() {
  var prname = document.getElementById("selectProduct");
...
function advSearchProduct(x) {
  ...
  request.open("POST", "advSearchProducProcess.php", true);
  request.send(f);
}
  ...
  request.onreadystatechange = function () {
    if ((request.readyState == 4) & (request.status == 200)) {
      ...
      if (response == "empty") {
        ...
        swal("Please Select Your Profile Picture ", response, "info");
      } else {
...
function doCheckout(payment, path) {
  ...
  payhere.onCompleted = function onCompleted(orderId) {
    ...
    // Note: validate the payment and show success or failure page to the
...
function buyNow(stockId) {
  ...
  if (qty.value > 0) {
    ...
    alert("Pelase Enter valid Quantity");
  }
}
